...
@Component({
  selector: 'app-employee-search-by-dropdown',
  templateUrl: './employee-search-by-dropdown.component.html',
  styleUrls: ['./employee-search-by-dropdown.component.css']
})
export class EmployeeSearchByDropdownComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.retrivingIdWithAllNullValues();
  ...
  retrivingIdWithAllNullValues()
  {
    this.employeedataservice.lastEmployeeIdToBePresent().subscribe(
      data=>{
        this.idWithNullValues=data
        console.log(data);
  ...
  Search() {
    ...
    this.employeedataservice.getCustomEmployee(this.req).subscribe(
      data => {
        this.employees = data;
        this.totalSalary=this.employees.reduce((sum,employee)=>sum+employee.salary,0)
  ...
  SearchByPaging() 
  {
    ...
    this.employeedataservice.GetAllEmployeesByPaging(this.req,this.currentPage,this.pageSize).subscribe(
      data => {
        ...
        //this.sortEmployeeBySalary();
        console.log(data);
        this.employeedataservice.getAllEmployeeTotalSalary(this.req).subscribe(
          value=>{
            this.totalSalary=value;
            this.totalEmployee=data.totalElements
...
SearchExcel() {
  ...
  this.employeedataservice.getCustomEmployee(this.req).subscribe(
    data => {
      this.excelEmployee = data;
      //this.sortByName();
      //this.sortEmployeeBySalary();
      this.employeedataservice.exportToExcel(this.excelEmployee,'Employee_Data')
      console.log(data);
...
downloadPDF(){
  ...
  this.employeedataservice.getCustomEmployee(this.req).subscribe(
    data=>{
      ...
      const col =['ID','NAME','DEPARTMENT','ADDRESS','SALARY'];
      const rows:any[]=[]
       data.forEach(emp=>{
        const temp= [emp.id,emp.name,emp.department,emp.address,emp.salary];
        rows.push(temp);
       });
...
export class EmployeeVehicleDTO{
  constructor(
    public id:number,
    public name:String,
