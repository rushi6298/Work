...
import { EmployeeVehicleDTO } from '../employee-search-by-dropdown/employee-search-by-dropdown.component';

@Component({
  selector: 'app-typed-query-paging',
  templateUrl: './typed-query-paging.component.html',
  styleUrls: ['./typed-query-paging.component.css']
})
export class TypedQueryPagingComponent implements OnInit {
  ...
  pageNumber: number = 1; // Current page number (1-based)
  pageSizes: number[] = [5, 10, 15, 20]; // Available page sizes
  ...
  SearchByPaging() {
    ...
    if(this.req.department.length===0)
    {
      console.log(this.req);
    }

    this.req.department=this.d
    
    

    this.employeedataservice.GetAllEmployeeByBackendPaginationWithTypedQuery(this.req,this.currentPage,this.pageSize).subscribe(
      data => {
        this.employeeDto = data.content;
...
// getAll EmployeeVehicle by paging and typed query

SearchByEmployeeVehiclePaging() {
  console.log("search by paging");
